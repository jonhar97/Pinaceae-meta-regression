...
// interaction population fragmentation level and life-history 
 ...
 for (j in 1:N)
    theta[j] = mu + a[group[j]] + b[group_sp[j]] + alpha*F[j] + beta[incidence_L[j]] + gamma[incidence_G[j]] + delta[incidence_LG[j]] + epsilon[incidence_L[j]]*F[j]; // //*delta0[incidence_L[j]]; //  b[group_sp[j]] + c[group_lg[j]] + 
...
model {
 ...
 // for (k in 1 : (K-1)) 
  ...
  alpha ~ normal(0, 0.1);	
  for (k in 1 : (J-1))
    beta0[k] ~ normal(0, 0.1); // vague priors 
  for (k in 1 : (M-1))
    gamma0[k] ~ normal(0, 0.1); // vague priors 
  for (k in 1 : (JM-1))
    delta0[k] ~ normal(0, 0.1); // vague priors 
  for (k in 1 : (J-1))
    epsilon0[k] ~ normal(0, 0.1); // vague priors 
  for (j in 1:L)
